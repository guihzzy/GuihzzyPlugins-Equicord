...
import { showVoiceNotification, teardownVoiceNotifications } from "./components/VoiceChannelNotification";
...
function log(entry: Omit<VoiceChannelLogEntry, "timestamp">) {
    ...
    if (settings.store.showNotifications) {
        if (entry.type === "move") {
            if (isMyChannel(entry.newChannelId ?? undefined)) {
                showVoiceNotification({ ...entry, type: "join", channelId: entry.newChannelId! });
            } else if (isMyChannel(entry.oldChannelId ?? undefined)) {
                showVoiceNotification({ ...entry, type: "leave", channelId: entry.oldChannelId! });
            }
        } else {
            showVoiceNotification(entry);
        }
    }
}
